...
- **AC-RS-9 — History.** Every row's `source` resolves to a file and line at the named SHA; `docs/history/corpus-<sha>/` holds the old corpus byte-identical to that SHA — the 62 in-scope files less `skills/directive-invariants.md`, a tool input that moves to `process/directive-invariants.md`; `git log --all` shows no rewritten ref.
