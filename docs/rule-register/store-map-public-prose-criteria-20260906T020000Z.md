# Store map — topic `public-prose-criteria`

Source file: `public-prose-criteria.md` @ fd54448. Register rows R0812–R0861 (50 rows).
Directive: docs/cycles/store-all-topics-20260906T020000Z.md.

| register id | source file | disposition | store id |
|---|---|---|---|
| R0812 | public-prose-criteria.md | retired-ruling (container rule 4: which documents to read and when) | — |
| R0813 | public-prose-criteria.md | retired-ruling (store-fix-1 item 4: describes) | — |
| R0814 | public-prose-criteria.md | retired-ruling (container rule 4: precedence of the Voice document over this one) | — |
| R0815 | public-prose-criteria.md | written | R0815 |
| R0816 | public-prose-criteria.md | retired-ruling (store-fix-1 item 4: describes) | — |
| R0817 | public-prose-criteria.md | written | R0817 |
| R0818 | public-prose-criteria.md | written | R0818 |
| R0819 | public-prose-criteria.md | written | R0819 |
| R0820 | public-prose-criteria.md | written | R0820 |
| R0821 | public-prose-criteria.md | written | R0821 |
| R0822 | public-prose-criteria.md | written | R0822 |
| R0823 | public-prose-criteria.md | written | R0823 |
| R0824 | public-prose-criteria.md | merged into R0821 | R0821 |
| R0825 | public-prose-criteria.md | merged into R0821 | R0821 |
| R0826 | public-prose-criteria.md | written | R0826 |
| R0827 | public-prose-criteria.md | written | R0827 |
| R0828 | public-prose-criteria.md | merged into R0821 | R0821 |
| R0829 | public-prose-criteria.md | written | R0829 |
| R0830 | public-prose-criteria.md | written | R0830 |
| R0831 | public-prose-criteria.md | written | R0831 |
| R0832 | public-prose-criteria.md | written | R0832 |
| R0833 | public-prose-criteria.md | written | R0833 |
| R0834 | public-prose-criteria.md | merged into R0833 | R0833 |
| R0835 | public-prose-criteria.md | written | R0835 |
| R0836 | public-prose-criteria.md | written | R0836 |
| R0837 | public-prose-criteria.md | written | R0837 |
| R0838 | public-prose-criteria.md | written | R0838 |
| R0839 | public-prose-criteria.md | written | R0839 |
| R0840 | public-prose-criteria.md | written | R0840 |
| R0841 | public-prose-criteria.md | merged into R0840 | R0840 |
| R0842 | public-prose-criteria.md | merged into R0840 | R0840 |
| R0843 | public-prose-criteria.md | merged into R0840 | R0840 |
| R0844 | public-prose-criteria.md | written | R0844 |
| R0845 | public-prose-criteria.md | written | R0845 |
| R0846 | public-prose-criteria.md | written | R0846 |
| R0847 | public-prose-criteria.md | retired-ruling (C153 cut) | — |
| R0848 | public-prose-criteria.md | merged into R0820 | R0820 |
| R0849 | public-prose-criteria.md | written | R0849 |
| R0850 | public-prose-criteria.md | retired-ruling (store-fix-1 item 4: describes) | — |
| R0851 | public-prose-criteria.md | written | R0851 |
| R0852 | public-prose-criteria.md | retired-ruling (ruling 5: negation of R0851) | — |
| R0853 | public-prose-criteria.md | written | R0853 |
| R0854 | public-prose-criteria.md | written | R0854 |
| R0855 | public-prose-criteria.md | merged into R0853 | R0853 |
| R0856 | public-prose-criteria.md | merged into R0853 | R0853 |
| R0857 | public-prose-criteria.md | merged into R0853 | R0853 |
| R0858 | public-prose-criteria.md | merged into R0853 | R0853 |
| R0859 | public-prose-criteria.md | merged into R0853 | R0853 |
| R0860 | public-prose-criteria.md | merged into R0853 | R0853 |
| R0861 | public-prose-criteria.md | merged into R0853 | R0853 |

Counts: rows consumed 50; rows written 31; definitions 7; merged away 15; split 0; retired 4 (R0812, R0814 container rules; R0847 C153 cut; R0852 negation).

Notes on the rulings applied:

- **C153 cut** (directive, DIVERGENT CLUSTERS, Dave's ruling 2026-09-05): neither
  R0847 ("where discoverability and readability conflict, readability wins") nor
  the Copy Editor's R0978 is written. R0847 is retired here; the Copy Editor
  topic retires R0978.
- **C150** (R0820, R0848): one obligation — the register is not altered to serve
  a secondary goal — with both of its triggers in the body. Written at R0820.
- **C022** (R0821–R0825, R0828): R0821 carries the tier-legibility rule with the
  four tier names, the verb-and-frame mechanism (R0824, R0825) and the
  never-by-narrated-self-correction form (R0828). R0822 (attribute a relayed
  claim) and R0823 (show the work for a demonstrated claim) state obligations
  beyond tier legibility, with their own triggers, and are written separately
  under ruling 3.
- **C041** (R0840–R0843): one row, the three cuts listed in the body.
- **C015** (R0855–R0861): merged, with the seven tells listed in the body, into
  R0853 — the row that states the obligation the list is a class under
  (clusters artifact, method note 4). R0853 is the lowest id of the merge.
- **C007, C033, C066, C074, C147, C148, C149, C151, C152, C154, C155, C156,
  C157** are cross-file agreeing clusters. Under ruling 1 no cluster-mate in
  voice.md, copy-editor.md, critic.md, writer.md, outline.md or LEXICON.md was
  merged in or quoted; every body here is written from this file's text only.
- **DEC-000380**: nothing in this file retires under it, as the digest card
  predicted.

Store fix pass 2 item 7 strips `role`, `session` and `corpus` from 2 define rows (R0836, R0845), which now carry `term` and nothing else — the bundle tool pulls them by scanning selected bodies for their terms; and re-verbs 2 `define` rows carrying a null term (R0826, R0838), each body restated as the instruction it carries.
