# Land change-flow revision 4 and read the whole document
...
docs/cycles/change-flow-revision-4-read-20260905T225000Z.md in that worktree,
commit it alone with a message naming the package it opens
(`process: change-flow revision 4 — directive`), push the branch
change-flow-r4 to origin with a plain `git push origin change-flow-r4`, never
...
WORKING-TREE DISPOSITION (exclusive assignment): this session works only in a
worktree at "$TMPDIR/fiducial-change-flow-r4", created by:
git worktree add --no-track "$TMPDIR/fiducial-change-flow-r4" -b change-flow-r4 origin/change-flow-r3

BASE VERIFICATION

Before anything else, fetch origin/change-flow-r3 and origin/main and confirm
the base — the branch change-flow-r3, not main — is at the reviewed ref
d62d1ebbd7cf1815410eb7b68484ca6d16592b75, and that origin/main is at
...
- reviews/change-flow-read-3-20260905T214500Z.md @ d62d1ebbd7cf1815410eb7b68484ca6d16592b75
  — the third read; this session checks closure of its F-1 through F-10 and
  reads the whole document fresh. The first two reads
  (reviews/change-flow-read-20260905T195000Z.md,
  reviews/change-flow-read-2-20260905T204500Z.md, same revision) are context
  only
...
- docs/rule-register/rule-register-20260904T210000Z.md @ 45b02c7b88a0e08f4872713aa39f840a59117423
  — every row revision 4 cites by `R` id, and every row in its closing
  section's restated and retired lists; do not read it whole
...
Two acts, two commits, in order, on the branch change-flow-r4, cut from the
branch change-flow-r3. This is the fourth read of process/change-flow.md and a
whole-document read, as the third was. You did not draft the revision; the
decision session did. Dave ruled the third read's F-2: the entry condition is
kept.

ACT 1 — land revision 4. Overwrite process/change-flow.md with the following,
verbatim. Commit it alone with the
message `process: change-flow — revision 4 after the third read`. Push.
Report the SHA.
...
ACT 2 — the read. Write reviews/change-flow-read-4-20260905T225000Z.md in the
review-artifact schema, amended as the document states: one overall `Verdict:`
in the header, most severe of the passes, and each pass section opening
with a `Verdict (<pass>):` line. The header line reads
"# Review: process/change-flow.md — cycle 4". Review the whole document at
the ACT 1 SHA.

- **Closure.** For each of the third read's F-1 through F-10, one line: closed
  (with the line in revision 4 that closes it) or open (with why). A finding
  closed by wording that introduces a new contradiction is open.
- **Continuity.** The closing section now has three lists — amended,
  restated, retired — and an "unchanged" sentence. Check each row in each
  list against the register and the body: an amended row's change appears in
  the body; a restated row's obligation survives in the body with only its
  vocabulary changed; a retired row's obligation is the status transition and
  nothing in the body depends on it; no row the body changes is missing from
  the lists. Then: does any sentence contradict a decision in decisions/log.md
...
message `reviews: change-flow read 4`. Push. Report both SHAs and the verdicts.
Then remove the worktree with `git worktree remove "$TMPDIR/fiducial-change-flow-r4"`
...
~~~sh
{
  git -C "$TMPDIR/fiducial-change-flow-r4" diff --stat origin/change-flow-r3..HEAD
  git -C "$TMPDIR/fiducial-change-flow-r4" log --oneline origin/main..HEAD
  grep -c '^## ' process/change-flow.md
  grep -c '^| \*\*' process/change-flow.md
} 2>&1 | tee "$TMPDIR/change-flow-r4-verify.log"
~~~

Expected: diff-stat against origin/change-flow-r3 names three files — this
directive and the review artifact as additions, process/change-flow.md as a
modification; log against origin/main shows exactly twelve commits (nine
already on change-flow-r3 plus the three this session lands, the directive
commit included); 7 `##` headings; 4 table rows beginning with a bold cell.

STOP CONDITIONS

Pinned to the reviewed ref d62d1ebbd7cf1815410eb7b68484ca6d16592b75. Cannot execute as written: stop
...
or an author-region marking.
    ...
    Land change-flow revision 4 and read the whole document — skills/directive-invariants.md @ 4c9cd22b01d3387bbc4d62e20a7d26bc5e0ab035
