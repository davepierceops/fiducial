Write this directive verbatim to docs/cycles/directive-authoring-naming-forms-<timestamp>.md — generate the timestamp yourself, ISO 8601 basic format, UTC — then commit and push it as your first act per the landing sequence below, and report the SHA read back from git.
...
WORKING TREE — exclusive assignment. Other sessions hold the main clone and several worktrees. From the clone root: git worktree add "$TMPDIR/fiducial-da-naming" origin/main — then do all work in that directory and nowhere else. In it: git checkout -b directive-authoring-naming-forms. Push without -u; upstream tracking cannot be set from a $TMPDIR worktree.
...
Content commit — exactly one commit touching exactly one tracked path, skills/directive-authoring.md. In "Naming", replace the sentence "A directive file is `docs/cycles/<descriptor>-<timestamp>.md`, the timestamp in ISO 8601 basic format." with, verbatim:
...
The same commit flips frontmatter to status: in-review and last-reviewed: null, per the policy's revision-lifecycle rule. The body diff must stay at or under ten changed lines; if the replacement as dictated exceeds that, stop and surface — do not compress the wording yourself. Push, verify by ls-remote, report the commit SHA and the full diff.
...
1. Append one line to reviews/expedited-log.md naming skills/directive-authoring.md, the content-commit SHA, today's date, and what changed (numbered cycle-directive form licensed alongside the timestamp form). Its own commit.
