Write this directive verbatim to docs/cycles/directive-authoring-tree-disposition-<timestamp>.md — generate the timestamp yourself, ISO 8601 basic format, UTC — then commit and push it as your first act per the landing sequence below, and report the SHA read back from git.
...
WORKING TREE — exclusive assignment. Other sessions hold the main clone and several worktrees. From the clone root: git worktree add "$TMPDIR/fiducial-da-amendment" origin/main — then do all work in that directory and nowhere else. In it: git checkout -b directive-authoring-tree-disposition. Push without -u; upstream tracking cannot be set from a $TMPDIR worktree.
...
Content commit — exactly one commit touching exactly one tracked path, skills/directive-authoring.md. In "Writing the directive file", replace the bullet beginning "**Exclusive working trees for split directives.**" with, verbatim:
...
1. Append one line to reviews/expedited-log.md naming skills/directive-authoring.md, the content-commit SHA, today's date, and what changed (working-tree disposition made mandatory for every directive). Its own commit.
