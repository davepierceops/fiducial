# Land process/change-flow.md as agreed
...
docs/cycles/change-flow-agreed-20260906T001000Z.md in that worktree, commit it
alone with a message naming the package it opens
(`process: change-flow agreed — directive`), push the branch change-flow-r6 to
origin with a plain `git push origin change-flow-r6`, never with `-u`, and
report the SHA. Do this before reading anything else and before touching any
other file. Base verification below runs before this act.
...
WORKING-TREE DISPOSITION (exclusive assignment): this session works only in a
worktree at "$TMPDIR/fiducial-change-flow-r6", created by:
git worktree add --no-track "$TMPDIR/fiducial-change-flow-r6" -b change-flow-r6 origin/change-flow-r5

BASE VERIFICATION

Before anything else, fetch origin/change-flow-r5 and origin/main and confirm
the base — the branch change-flow-r5, not main — is at the reviewed ref
9a75ec3d468f6f1d8dfdcdb81e79d9081ca21b57, and that origin/main is at
...
- process/change-flow.md @ 90fa09f62b28039a6fca684702aaaeeb741e6fc7 — the
  revision this one overwrites; read it so the diff you commit is the one you
  expect

TASK

One act, one commit. Dave agreed process/change-flow.md on 2026-09-05 after
five frontier reads, ruling that the row-level list at its end is cut and no
further read is run. This session lands the agreed text; it does not review
it, edit it, or improve it.

Overwrite process/change-flow.md with the following, verbatim. Commit it
alone with the message `process: change-flow — agreed 2026-09-05`. Push.
Report the SHA. Then remove the worktree
(`git worktree remove "$TMPDIR/fiducial-change-flow-r6"`) and report that it
is gone. Do not open a pull request.
...
process/ is outside the frontmatter hook's in-scope set; the hook should not
fire. If it does, stop and report; do not bypass it.
...
~~~sh
{
  git -C "$TMPDIR/fiducial-change-flow-r6" diff --stat origin/change-flow-r5..HEAD
  git -C "$TMPDIR/fiducial-change-flow-r6" log --oneline origin/main..HEAD | wc -l
  grep -c '^## ' process/change-flow.md
  grep -c 'R[0-9][0-9][0-9][0-9]' process/change-flow.md
} 2>&1 | tee "$TMPDIR/change-flow-r6-verify.log"
~~~

Expected: two files in the diff-stat — this directive added, process/
change-flow.md modified; 17 commits ahead of origin/main; 7 `##` headings;
the register-id line count is reported as observed (the body still cites
rows as pointers; the closing section cites none).

STOP CONDITIONS

Pinned to the reviewed ref 9a75ec3d468f6f1d8dfdcdb81e79d9081ca21b57. Cannot execute as written: stop
...
or an author-region marking.
    ...
    Land process/change-flow.md as agreed — skills/directive-invariants.md @ 4c9cd22b01d3387bbc4d62e20a7d26bc5e0ab035
