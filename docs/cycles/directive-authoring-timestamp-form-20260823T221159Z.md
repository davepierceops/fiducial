Write this directive verbatim to docs/cycles/directive-authoring-timestamp-form-<timestamp>.md — generate the timestamp yourself, ISO 8601 basic format, UTC — then commit and push it as your first act per the landing sequence below, and report the SHA read back from git.
...
WORKING TREE — exclusive assignment. Other sessions hold the main clone and several worktrees. From the clone root: git worktree add "$TMPDIR/fiducial-da-timestamp" origin/main — then do all work in that directory and nowhere else. In it: git checkout -b directive-authoring-timestamp-form. Push without -u; upstream tracking cannot be set from a $TMPDIR worktree.
...
Content commit — exactly one commit touching exactly one tracked path, skills/directive-authoring.md. In "Naming", replace the phrase "the timestamp in ISO 8601 basic format" with, verbatim:

the timestamp in ISO 8601 basic format with date and time components both present (as `20260820T161541`)

The sentence's surrounding text, including the reviewer-gated-cycle exception, stays untouched. The same commit flips frontmatter to status: in-review and last-reviewed: null, per the policy's revision-lifecycle rule. The body diff must stay at or under ten changed lines; if the replacement as dictated exceeds that, stop and surface — do not compress or reflow beyond what the substitution itself requires. Push, verify by ls-remote, report the commit SHA and the full diff.
...
1. Append one line to reviews/expedited-log.md naming skills/directive-authoring.md, the content-commit SHA, today's date, and what changed (timestamp form requires date and time components both present). Its own commit.
