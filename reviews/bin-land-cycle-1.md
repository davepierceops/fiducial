...
Evidence: Verified by running, in this session's sandbox: `git ls-remote origin bin-land-spec`, `git fetch origin main`, and `git push origin bin-land-spec` each exited 0 while writing `fatal: failed to store: 100001` to stderr. All three are *observed*. The ref-updating push that landed this branch emitted the same line and updated the ref, confirmed by `ls-remote`. Whether any git operation ever exits non-zero *because* of the credential helper is *unknown*; no failure was induced to find out.
Consequence: On the observed evidence git's exit code is already correct across fetch, push, and `ls-remote`, so a tool that consults exit status and then verifies has no use for the string at all. Requiring it to recognise the literal puts one sandbox's error text into the implementation — the environment lore this whole effort exists to move out of prose, relocated into code, where it is at least tested and still wrong the day the credential helper changes its message. The spec compounds this by not saying which signal decides: AC-LAND-04 asserts that success "is not derived from stderr content in either direction" while G4 requires the string to be recognised, and those two cannot both be operative.
