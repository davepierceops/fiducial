...
## Loose ends filed 2026-09-10 — AC-RS-8/9 and prose-criteria-rows reads

Non-blocking findings and owed items from the 2026-09-09 reads, filed per
DEC-000700 rather than reopening either delta.

- **MANIFEST.md's file-registry paragraph is stale** (Q-1, non-blocking). Location: MANIFEST.md:14-20, the paragraph "**The file registry.**". It tells a reader that the in-scope set is computed from `policies/document-metadata-policy.md` via `bin/check-frontmatter --all`; the AC-RS-8/9 branch deletes that tool and moves the policy to `docs/history/corpus-1527970.../`, so a reader following the paragraph reaches a deleted command and a path that no longer resolves. Fix: rewrite the paragraph to name the history-corpus copy as the record and state that no tool computes the in-scope set any more, or retire the paragraph with the mechanism it describes. reviews/ac-rs-8-9-read-20260909T200000Z.md (Q-1).
- **OPEN-ITEMS.md (this file) carries 27 references to the four AC-RS-8/9-retired tools** (Q-2, observation). `grep -cE 'check-frontmatter|flip-agreed|install-hooks|migrate-frontmatter' OPEN-ITEMS.md` returns 27; most read as historical record, but a handful state present-tense mitigations ("run `check-frontmatter --all` after any edit") that can no longer be performed. Fix: none required by the AC-RS-8/9 package, whose sweep scoped fixes to bin/, process/ and README.md — worth a pass when this backlog is next triaged. reviews/ac-rs-8-9-read-20260909T200000Z.md (Q-2).
- **bin/tests/helpers.py:541 `AUTHORING_RELPATH` names the pre-move path and is now unused** (Q-4, non-blocking). The constant names `skills/directive-authoring.md`, which the AC-RS-8/9 branch moves to `docs/history/corpus-1527970.../skills/directive-authoring.md`; nothing reads the constant (`grep -rn 'AUTHORING_RELPATH' bin/tests/*.py` returns only the definition), so no test fails, but it will mislead the next author who reaches for it. Fix: delete the constant, or repoint it at the history-corpus path beside `INVARIANTS_RELPATH` on the line above. reviews/ac-rs-8-9-read-20260909T200000Z.md (Q-4).
- **The dead `bin/aimeta` `load_context` chain raises on a moved path — retire-later, per the human** (S-2, non-blocking). `bin/aimeta/cli.py:84-104` (`load_context`, `Context`), `bin/aimeta/scope.py:18` (`POLICY_RELPATH`), `bin/aimeta/repo.py:263` (`role_slugs`), `:287` (`disposition_paths`), and `bin/aimeta/frontmatter.py:257` (`validate`) are reachable only through `load_context`, which has no callers anywhere in live code or tests; `scope.load_globs` raises `ScopeError` because `POLICY_RELPATH` still names the pre-move root path for `policies/document-metadata-policy.md`, moved by this same branch. No entrypoint can reach the raising call today — the risk is latent, for the next author who calls `load_context` expecting it to work. Fix (Dave's ruling: retire-later, not now): retire the chain — `load_context`, `Context`, `scope.py` and its test, and repo's role/disposition helpers — in a follow-up package, rather than repointing `POLICY_RELPATH`. reviews/ac-rs-8-9-read-20260909T200000Z.md (S-2).
- **R0835's second sentence still disclaims the structure axis while R1013 still cites it as a structure-defect container** (F-6, non-blocking). F-1's Fix asked for a ruling on whether R0835's second sentence ("Length and structure are the author's call per piece.") belongs in a row cited as a defect container; amendment B ruled the first half of F-1 (the corrected four-ID citation list) and left this half unruled — R0835's body is unchanged at rules/R1013.md:14 / rules/R0835.md:14. A Critic following R1013 to the first row in its structure-defect list still lands on a sentence disclaiming that axis. Fix: rule on whether the sentence stays — if it stays, R1013's "every structure defect in" phrasing may want different wording for that row; if it goes, R0835 becomes a clean restatement rule. reviews/prose-criteria-rows-read-2-20260909T230000Z.md (F-6).
- **R1605's co-render constraint passes vacuously for a citing row with no role/session/corpus key, and would not have caught the citation that motivated it** (F-7, non-blocking). Location: rules/R1605.md:14. 78 of 455 rows carry no `role` key at all (R0038 is one); such a row satisfies "every value the citing row carries" for any cited row, because it carries none — so R0038 citing R0821 (the pairing F-2 raised and this delta resolved by deleting the citation) would pass R1605's test even though the two cannot co-render under any `--where` selection. Second gap: `parse_where`/`--where` accept any key, and R1605 only tests three of them, so e.g. `--where topic=critic` returns citing and cited rows that share no tested key and still don't co-render. Fix: two directions for the decision session — quantify the rule over every selector key rather than enumerating three, and either forbid a keyless row from citing at all (matching R0845's exclusion from R1013) or require such a row cite only within its own topic. reviews/prose-criteria-rows-read-3-20260910T003000Z.md (F-7).
- **The `source:` line's SHA length is still split across the four cycle-sourced rows** (O-10, observation). rules/R1604.md:9 and rules/R1605.md:10 now carry `path:1 @ <7-char SHA>`, the ruled form; rules/R1601.md and rules/R1602.md, named as the model for that form, still carry 40-character SHAs and were outside this delta's blast radius. The store's dominant form (451 of 455 rows) is the 7-character form, so R1604/R1605 agree with the store and disagree with the two rows the ruling pointed at; nothing breaks, all four values resolve by hand. Fix: none owed by this package — if the store wants one form, a delta that owns R1601 and R1602 brings them in line. reviews/prose-criteria-rows-read-2-20260909T230000Z.md (O-10).
- **process/outline.md and process/voice.md still direct the writing roles to read the Public Prose Criteria as a document, though pull request #343 moved the criteria into rows the bundle already carries**. process/outline.md:12 ("Read the Public Prose Criteria and the Voice document in this bundle first"); process/outline.md:30 (citing "the Criteria's claims taxonomy — relayed, demonstrated, grounded, opinion" as if the Criteria were a document with a section to cite); process/voice.md:11 ("The author the Public Prose Criteria in this bundle are applied to. Read with the Criteria; where the two speak to the same thing, this document wins."). R1604 is now the read-first row for the writing roles (topic voice: "Read the Voice document before you write, edit, or read the piece, every time"), and the Criteria rows (topic public-prose-criteria) already reach those roles as bundle members — no reading instruction is needed for them. Fix: reword the three lines to stop directing a read of a document that no longer exists. Owed since #343 landed R1604 in place of R1161/R0953/R1001. reviews/prose-criteria-rows-read-3-20260910T003000Z.md (named on its "Not inspected" line as the process delta, out of that directive's scope).
- **Near-identical rows keyed separately to writer, copy-editor and critic want the same merge R1604 already got**. R1161, R0953 and R1001 — three copies of "read the Public Prose Criteria," one per role — were merged into one row, R1604, carrying all three roles (pull request #343). A sweep of the store for the same pattern — near-identical rows differing only in which of writer/copy-editor/critic they're keyed to — is owed, to merge each such cluster into one multi-role row the way R1604 was produced. reviews/prose-criteria-rows-read-3-20260910T003000Z.md (named on its "Not inspected" line as the parallel-row sweep, out of that directive's scope).
- **bin/bundle needs a branch-render mode so a delta's rendered bundles can be checked before merge**. bin/bundle:14 — `--where` refuses (exit 2, one line on stderr, nothing written) whenever HEAD is not origin/main. Both prose-criteria-rows reads had to read reach and co-render claims from `select()` in bin/rulestore/query.py against frontmatter keys rather than from a rendered bundle, because of this refusal. Fix: add a branch-render mode so a delta's rendered bundles can be checked before merge. reviews/prose-criteria-rows-read-3-20260910T003000Z.md (named on its "Not inspected" line as the bin/bundle branch-render gap, out of that directive's scope).
- **The `gh release create` line bin/release prints fails when run outside the repo directory**. bin/release:158 prints the command with no `-R <owner/repo>` flag, so `gh` cannot resolve which repository to target unless the command is run from inside a checkout. Fix: add `-R davepierceops/fiducial` to the printed line. Told directly by the human in this package's directive (docs/cycles/flush-20260910-20260910T033000Z.md); no companion review names it, and the printed command was read at bin/release:158 to confirm.
